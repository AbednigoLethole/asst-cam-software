...
class MyDriver : public canopen::FiberDriver {
 ...
 private:
  chrono::time_point<std::chrono::system_clock> rpdoStart, rpdoEnd;
  ofstream outFile;
 ...
 private:
  ...
  OnConfig(std::function<void(std::error_code ec)> res) noexcept override {
    try 
    {
      ...
      double azimuth = 45.0;
      Wait(AsyncWrite<double>(0x2000, 2, std::move(azimuth)));
      double elevation = 50.0;
      Wait(AsyncWrite<double>(0x2000, 3, std::move(elevation)));

      rpdoStart = chrono::high_resolution_clock::now();
      outFile.open("MasterPosition.csv");
  ...
  OnRpdoWrite(uint16_t idx, uint8_t subidx) noexcept override {
    ...
    if (idx == 0x2001)
    {
      ...
      rpdoEnd = chrono::high_resolution_clock::now();
      chrono::duration<double> elapsedTime = rpdoEnd - rpdoStart;
      rpdoStart = rpdoEnd;
      cout << "Current time - Previous time = " << elapsedTime.count() << " s\n"; 
      switch (subidx)
      {
        case 1:
          timestamp = rpdo_mapped[0x2001][1];
          cout << fixed << "Actual position:Timestamp = " << timestamp << "\n";
          break;
        
        case 2:
          azimuth = rpdo_mapped[0x2001][2];
          cout << fixed << "Actual position:Azimuth = " << azimuth << "\n";
          break;
        
        case 3:
          elevation = rpdo_mapped[0x2001][3];
          cout << fixed << "Actual position:Elevation = " << elevation << "\n";

          outFile << fixed << timestamp << ", " << azimuth << ", " << elevation << "\n";
          break;
      }
    }
  }
